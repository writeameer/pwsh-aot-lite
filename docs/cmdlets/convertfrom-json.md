...
| Object materialization | `ConvertFromJsonCommand.cs:122-134`; `../../.upstream/PowerShell/src/Microsoft.PowerShell.Commands.Utility/commands/utility/WebCmdlet/JsonObject.cs:169-466` | shared-substrate replacement | implemented J0 `AotJsonCodec` to closed `AotValue`; no cmdlet adapter | Upstream constructs `PSObject`/hashtable shapes through Newtonsoft and the PowerShell engine. | `json-closed-value-codec` | J0 managed/native codec proof passed; command lifecycle/output oracle remains pending |
| Duplicate/case-colliding/unsafe object keys | `JsonObject.cs:101,246-289`; stock `pwsh` 7.6.6 rejects differing-case keys, accepts an exact duplicate with its latter value, and accepts a whitespace-only name under the default object route | deferred | initial closed-record route rejects exact/case-colliding/blank/**whitespace-only** keys with `AOT6304` | `AotRecord` is ordinal-ignore-case and deliberately forbids blank/whitespace field names. Exact duplicates and whitespace-only names are explicit fail-closed variances; `-AsHashtable` remains wholly unsupported. | `json-case-collision`, `json-whitespace-key` | future stock/native exact, differing-case, nested, empty-key, and whitespace-key fixtures; the whitespace oracle must preserve stock success versus native `AOT6304` |
