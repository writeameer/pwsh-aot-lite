...
namespace System.Management.Automation.Language
{
    ...
    public sealed class Parser
    {
        ...
        private StatementAst ConfigurationStatementRule(IEnumerable<AttributeAst> customAttributes, Token configurationToken)
        {
            ...
            return AotExcludedConfigurationStatementRule(configurationToken);
#else
        ...
        /// <summary>
        /// Consumes a DSC configuration declaration without running the DSC
        /// parser-time loader.  The declaration remains an error AST, so it
        /// cannot silently become an ordinary supported statement.
        /// </summary>
        private StatementAst AotExcludedConfigurationStatementRule(Token configurationToken)
        {
            ReportError(
                configurationToken.Extent,
                AotLanguageDiagnostics.ConfigurationExcludedErrorId,
                AotLanguageDiagnostics.ConfigurationExcludedMessage);

            Token endToken = configurationToken;
            ...
            while (true)
            {
                ...
                else if (sawOpeningBrace && token.Kind == TokenKind.RCurly && --braceDepth == 0)
                {
                    break;
                }
            }

            return new ErrorStatementAst(ExtentOf(configurationToken, endToken), configurationToken);
        }
