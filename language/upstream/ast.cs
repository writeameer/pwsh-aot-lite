...
#nullable restore
    ...
    public class ConfigurationDefinitionAst : StatementAst
    {
        ...
        {
#if PWSH_AOT_LANGUAGE_EXCLUDE_DSC
            throw System.Management.Automation.AotExecutionBoundary.Create();
#else
            // **************************
    ...
    public class AttributedExpressionAst : ExpressionAst, ISupportsAssignment, IAssignableValue
    {
        ...
        {
            // PWSH-AOT-LANGUAGE-EXCLUSION: this member is the upstream
            // expression-compiler path, not AST construction or traversal.
            // The Native AOT language host deliberately has no compiler or
            // runtime binder; a later lowerer owns supported execution.
            throw System.Management.Automation.AotExecutionBoundary.Create();
        }

        Expression IAssignableValue.SetValue(Compiler compiler, Expression rhs)
        {
            // PWSH-AOT-LANGUAGE-EXCLUSION: see GetValue above.
            throw System.Management.Automation.AotExecutionBoundary.Create();
        }
...
#nullable restore
    ...
    public sealed class GenericTypeName : ITypeName, ISupportsTypeCaching
    {
        ...
        {
#if PWSH_AOT_LANGUAGE_EXCLUDE_DSC
            // A structural type-name node is useful to the parser/lowerer, but
            // materializing arbitrary closed generic Types is reflection/type
            // execution and is not Native-AOT safe.
            throw System.Management.Automation.AotExecutionBoundary.Create();
#else
            if (_cachedType == null)
            {
    ...
    public sealed class ArrayTypeName : ITypeName, ISupportsTypeCaching
    {
        ...
        {
#if PWSH_AOT_LANGUAGE_EXCLUDE_DSC
            // Keep array type-name grammar/traversal while excluding runtime
            // Type.MakeArrayType materialization from the AOT host.
            throw System.Management.Automation.AotExecutionBoundary.Create();
#else
            try
            {
    ...
    public class VariableExpressionAst : ExpressionAst, ISupportsAssignment, IAssignableValue
    {
        ...
        Expression IAssignableValue.GetValue(Compiler compiler, List<Expression> exprs, List<ParameterExpression> temps)
        {
            // PWSH-AOT-LANGUAGE-EXCLUSION: this member is invoked only by the
            // upstream dynamic expression compiler.
            throw System.Management.Automation.AotExecutionBoundary.Create();
        }

        Expression IAssignableValue.SetValue(Compiler compiler, Expression rhs)
        {
            // PWSH-AOT-LANGUAGE-EXCLUSION: variable assignment is lowered by
            // the future closed AotValue evaluator, never DynamicExpression.
            throw System.Management.Automation.AotExecutionBoundary.Create();
        }

        internal Type GetVariableType(Compiler compiler, out IEnumerable<PropertyInfo> tupleAccessPath, out bool localInTuple)
        {
            // PWSH-AOT-LANGUAGE-EXCLUSION: tuple layout is an implementation
            // detail of the excluded expression compiler.
